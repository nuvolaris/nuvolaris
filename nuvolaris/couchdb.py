...
def init_system(config):
    ...
    res = check(cu.add_user(config['controller']['user'], config['controller']['password']), "add_user: controller", res)
    return check(cu.add_user(config['invoker']['user'], config['invoker']['password']), "add_user: invoker", res)
...
def init_subjects(config):
    ...
    members = [config['invoker']['user'], config['controller']['user']]
...
def add_subjects(config):
    res = True
    db = "subjects"
    for name in config['openwhisk'].keys():
        [uuid, key] = config['openwhisk'][name].split(":")
